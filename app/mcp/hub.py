...
class McpHub:
    ...
    def all_tool_schemas(self, *, allow_k8s: bool = True) -> list[dict[str, Any]]:
        """KB tools plus optional K8s tools (caller gates allow_k8s on input grounding)."""
        schemas = self.kb_tool_schemas()
        if allow_k8s:
            schemas.extend(self.k8s_tool_schemas())
        return schemas
