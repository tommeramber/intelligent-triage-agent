...
from app.k8s.grounding import K8S_GROUNDING_BLOCK_REASON, input_has_k8s_grounding
...
- When Kubernetes MCP tools are available in a later iteration, the error text already
  names a concrete target (namespace, pod, or workload). You MAY use those tools after
  the KB lookup to gather read-only cluster evidence (pods, logs, events). Do not mutate
  the cluster.
- When Kubernetes MCP tools are not offered, the input lacks that concrete target —
  triage from the KB and error text only; do not invent namespaces or pod names.
...
def _append_kb_titles(target: list[str], entries: list) -> None:
...
async def _run_triage_loop(
    ...
    incident_text = f"{error_code}: {description}"
    k8s_input_grounded = input_has_k8s_grounding(incident_text)
    k8s_tools_allowed = hub.k8s_available and k8s_input_grounded

    if hub.k8s_available and k8s_input_grounded:
        logger.info(
            "Kubernetes MCP connected — cluster tools available after KB lookup (grounded input)"
        )
    elif hub.k8s_available:
        logger.info(
            "Kubernetes MCP connected but input lacks namespace/pod/workload grounding — "
            "K8s tools withheld"
        )
    ...
    docs_useful: list[str] = []
    kb_keyword_match = False
    evidence = EvidenceSources()
    k8s_invoked: list[str] = []
    k8s_workload_evidence = False
    k8s_access_denied = False
    ...
    for iteration in range(settings.agent_max_iterations):
        ...
        if iteration == last_iteration:
            ...
            tools = hub.all_tool_schemas(allow_k8s=k8s_tools_allowed)
            tool_choice = "auto"
            response = await client.chat.completions.create(
                model=settings.openai_model,
                messages=messages,
        ...
        if choice.finish_reason == "tool_calls" and assistant_msg.tool_calls:
            for tool_call in assistant_msg.tool_calls:
                ...
                if (
                    classified
                    and classified[0] == "kubernetes"
                    and not k8s_tools_allowed
                ):
                    result_str = json.dumps({"error": K8S_GROUNDING_BLOCK_REASON})
                else:
                ...
                if classified:
                    ...
                    if channel == "knowledge_base":
                        ...
                        if label not in k8s_invoked:
                            k8s_invoked.append(label)
                        outcome = classify_k8s_tool_result(label, result_str)
                        if outcome == K8sToolOutcome.WORKLOAD_EVIDENCE:
                            k8s_workload_evidence = True
                        elif outcome == K8sToolOutcome.ACCESS_DENIED:
                            k8s_access_denied = True
                ...
                if tool_name == KB_TOOL_NAME:
                    try:
                        ...
                        consulted = result_data.get("consulted_entries", [])
                        useful = result_data.get("useful_entries", [])
                        kb_keyword_match = bool(result_data.get("keyword_match", False))
                        logger.info(
                            "KB tool: keyword_match=%s consulted=%d useful=%d",
                            kb_keyword_match,
                            len(consulted),
                            len(useful),
                        )
                        _append_kb_titles(docs_consulted, consulted)
                        _append_kb_titles(docs_useful, useful)
        ...
        if choice.finish_reason in ("stop", "length"):
            ...
            evidence.kubernetes = finalize_kubernetes_evidence(
                k8s_invoked,
                workload_evidence=k8s_workload_evidence,
                access_denied=k8s_access_denied,
            )
            return TriageResponse(
                ...
                docs_useful=docs_useful,
                kb_keyword_match=kb_keyword_match,
