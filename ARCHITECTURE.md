...
### KB doc fields — tool-sourced, not LLM-claimed

`docs_useful`, `docs_consulted`, and `kb_keyword_match` come from **actual KB tool results**, not the LLM JSON. Code-only HTTP code matches may still be returned to the model as a weak `matched_entries` fallback, but they do **not** populate `docs_consulted` / `docs_useful`. When `kb_keyword_match` is false, no description keyword grounded any runbook.

- `docs_useful` — top keyword-grounded runbook title(s) (primary evidence).
- `docs_consulted` — up to three keyword-grounded titles (broader audit trail).
- `evidence_sources.kubernetes` — `invoked`, `status` (`not_invoked` | `obtained` | `no_accessible_evidence` | `access_denied`), `message`, and `evidence_obtained` (true only for workload evidence in an allowed namespace; allowlist-only calls do not count).
